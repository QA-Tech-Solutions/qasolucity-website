...
export interface Pathway {
  ...
  /** e.g. "From 4 weeks" — see shortestCertificationDurationLabel below. */
...
// Both pathways draw from the same certification catalog, and training
// length varies by which certification the buyer picks - so, like price,
// the card can only honestly show a "from" figure: the shortest training
// duration across the whole catalog. Derived from the catalog itself
// rather than hardcoded, so it can't drift out of sync if durations change.
function parseLeadWeeks(duration: string): number {
  const match = duration.match(/\d+/);
  return match ? parseInt(match[0], 10) : Infinity;
}

const shortestCertificationWeeks = Math.min(
  ...allCertifications.map((cert) => parseLeadWeeks(cert.duration))
);

export const shortestCertificationDurationLabel = `From ${shortestCertificationWeeks} weeks`;
...
export const pathways: Pathway[] = [
  {
    ...
    durationLabel: shortestCertificationDurationLabel,
    features: [
