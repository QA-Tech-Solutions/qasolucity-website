...
const TRACK_LABELS: Record<CertificationEnrollmentDetails["track"], string> = {
  prep: "Self-Starter Prep Track (Route A)",
  bundle: "All-Inclusive Certification Bundle (Route C)",
};

export function certificationInternalNotificationEmail(payload: CertificationEnrollmentDetails) {
  const { firstName, lastName, email, phone, track, certificationName, notes, priceNgn, voucherCode } =
    payload;
  ...
  const html = wrapper(`
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:4px;">
      ...
      ${detailRow(
        "Voucher",
        track === "bundle"
          ? escapeHtml(voucherCode ? `Auto-assigned: ${voucherCode}` : "None in stock — assign manually within 24h")
          : undefined
      )}
    </table>
    ${
  ...
  const text = [
    ...
    `Price: ${priceLabel}`,
    track === "bundle"
      ? `Voucher: ${voucherCode ? `Auto-assigned (${voucherCode})` : "None in stock — assign manually within 24h"}`
      : null,
    notes ? `\nNotes:\n${notes}` : null,
    "",
    `Reply directly to ${email} to follow up.`,
...
export function certificationConfirmationEmail(payload: CertificationEnrollmentDetails) {
  const { firstName, track, certificationName, priceNgn, voucherCode } = payload;
  const priceLabel = nairaFormatter.format(priceNgn);

  const prepSteps = [
    ["1", "Check your inbox", "Your class schedule and login details arrive separately, usually within a few hours."],
    [
      "2",
  ...
  const bundleSteps = [
    ["1", "Payment confirmed", "Your training seat and exam voucher are both locked in."],
    [
      "2",
      voucherCode ? "Your voucher code is ready" : "Your voucher is being issued",
      voucherCode
        ? `Your prepaid exam voucher code is <strong>${escapeHtml(voucherCode)}</strong>. Keep it safe — you'll enter it on the registrar's site when you book your test date.`
        : "Our team purchases and assigns your unique prepaid voucher code by hand, and emails it to you within 24 hours.",
    ],
    [
      "3",
